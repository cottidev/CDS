...
document.querySelectorAll('a, button, .pole-card, .member-card, .join-btn').forEach(el => {
...
class Particle {
  ...
  reset() {
    ...
    this.growing = true;
    this.color = Math.random() > 0.6 ? '#1aff7a' : Math.random() > 0.5 ? '#3b6bff' : '#00d4aa';
  }
  update() {
    this.x += this.speedX;
    this.y += this.speedY;
...
const revealObs = new IntersectionObserver((entries) => {
  entries.forEach((entry, i) => {
    if (entry.isIntersecting) {
      // Stagger child cards
      const cards = entry.target.querySelectorAll('.pole-card, .member-card, .join-btn');
